...
public class GetAQuotePage_W2CQA extends BasePage {
	...
	By leavingandReturnsamedate = By
			.xpath("//*[contains(text(),'Return Date cannot be the same as Leaving on Date.')]");
	...
	By departureDateMoreErrorMsg = By
			.xpath("//*[contains(text(),'Leaving On date cannot be date before the current date')]");
	...
	By singletripretutndate365days = By
			.xpath("//*[contains(text(),'Return date cannot be more than 365 days from the Leaving date')]");
	//By AMTAgebelow18and75year = By.xpath(
	 ...
	 By AMTAgebelow18and75year=By.xpath("//*[contains(text(),\"Traveller's age must be less than or equal to 75.\")]");
	By AMTretrundate60days = By
			.xpath("//*[text()=' Returning On date must be less than or equal to 60 days from the Leaving On date. ']");
	By returndateAMTerrormsg = By
			.xpath("//*[contains(text(),' Return Date cannot be the same as Leaving on Date. ')]");

	By departDateWhole = By.xpath("(//div[contains(@class,'row')])[4]/div[contains(@class,'col')][2]");

	public GetAQuotePage_W2CQA(WebDriver d) {
		super(d);
		waitForLoader();
		/*
		 * if
	...
	public void getleavingandreturndatesame(String traveldestination, String leavingOn, String returningOn,
		...
		d.findElement(destinationEntry).click();
		waitFor(3);
		d.findElement(departDate).clear();
		d.findElement(departDate).sendKeys(leavingOn);
		waitFor(3);
	...
	public void getleavingandreturndateAMT(String traveldestination, String leavingOn, String returningOn,
			String agePrimarytraveller, String agesecondaryTravel, String noofDependants) throws InterruptedException {
		waitForVisible(travelDestination);
		waitFor(3);
		d.findElement(travelDestination).sendKeys(traveldestination);
		waitFor(3);
